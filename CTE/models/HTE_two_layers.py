...
class HTE(nn.Module):
    def __init__(self, args, input_shape, device):
        ...
        self.word_calc2 = torch_CTE_layers.FernBitWord_tabular(args.Fern2['M'], args.Fern2['K'], args.Fern2['num_of_features'], args, device)
        VT2_input_shape = [input_shape[0], args.Fern2['M'], args.Fern2['K']]
        self.voting_table2 = torch_CTE_layers.FernSparseTable_tabular(args.Fern2['K'],
                                                              args.Fern2['M'],
    ...
    def forward(self, x):
        ...
        x = self.word_calc2(x)
        self.save_sec_fern_values = None
        if self.word_calc2.anneal_state_params['count_till_update'] == self.word_calc2.anneal_state_params['batch_till_update']:
            self.save_sec_fern_values = self.save_fern_values(self.word_calc2.bit_functions_values)
    ...
    def on_batch_ends(self, device):
        ...
        self.word_calc2.anneal_state_params, self.word_calc2.ambiguity_thresholds = update_ambiguity_thresholds_tabular(
                    self.word_calc2.anneal_state_params,
                    self.word_calc2.ambiguity_thresholds,
                    self.save_sec_fern_values,
                    device
                )

        self.word_calc2.anneal_state_params = update_Rho_tempature_tabular(self.word_calc2.anneal_state_params)
