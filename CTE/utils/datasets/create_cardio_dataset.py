...
def main(args=None):
    if os.path.isdir(args.datapath):
        ...
        cardio_data = pd.read_csv(os.path.join(args.datadir, 'cardio_train.csv'), sep=';')
        cardio_data = cardio_data.drop(['id'], axis='columns')
        classes = cardio_data['cardio']
        features = cardio_data.drop(['cardio'], axis='columns')
...
if __name__ == '__main__':
    ...
    args.datadir = os.path.join(GetEnvVar('DatasetsPath'), 'HTE_Omri_Shira', 'CARDIO')
