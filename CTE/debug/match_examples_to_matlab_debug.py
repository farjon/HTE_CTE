...
def main():
    ...
    args.word_calc_learning_rate = 5e-4
    ...
    # for index in range(1,3):
        ...
        for epoch in range(current_num_of_epochs):
            ...
            for i, data in enumerate(train_loader, 0):
                ...
                if i == 0:
                    images = np.zeros([2, 1, 28, 28])
                    images[0, 0, :, :] = np.array(
                        pd.read_csv(os.path.join(init_weights_base_dir, 'first_image.csv'), header=None))
                    images[1, 0, :, :] = np.array(
                        pd.read_csv(os.path.join(init_weights_base_dir, 'second_image.csv'), header=None))
                    tensor_n = torch.from_numpy(images).cuda()
                    labels = torch.tensor([6, 0]).cuda()
                else:
                    images = np.zeros([2, 1, 28, 28])
                    images[0, 0, :, :] = np.array(
                        pd.read_csv(os.path.join(init_weights_base_dir, 'third_image.csv'), header=None))
                    images[1, 0, :, :] = np.array(
                        pd.read_csv(os.path.join(init_weights_base_dir, 'fourth_image.csv'), header=None))
                    tensor_n = torch.from_numpy(images).cuda()
                    labels = torch.tensor([0, 3]).cuda()
                inputs_debug = tensor_n

                outputs, inter_outputs = model(inputs_debug)
