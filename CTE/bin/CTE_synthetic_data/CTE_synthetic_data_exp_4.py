...
def main():
    ...
    datapath_train = os.path.join(GetEnvVar('DatasetsPath'), 'CTE_synthetic data', 'Exp 4', 'train')
    datapath_test = os.path.join(GetEnvVar('DatasetsPath'), 'CTE_synthetic data', 'Exp 4', 'test')
    ...
    if args.loss == 'categorical_crossentropy':
        ...
        for epoch in range(args.num_of_epochs):
            ...
            if args.debug:
                ...
                plt.ylim(0, 1)
                plt.title('Experiment 4 Accuracy over epochs')
                plt.draw()
